...
export class Link implements Command {
    ...
    async run(bot: Discord.Client, cmdCtx: CommandContext): Promise<void> {
        cmdCtx.msg.reply("linking starts");

        // Checks command length
        if(cmdCtx.args.length != 3) {
        ...
        let parameter = cmdCtx.args[2];
        if(!(parameter == "-target" || parameter == "-master")) {
            new Embed(typeOfEmbed.basic, bot, cmdCtx.msg, color.gray, "Erreur de link",
                    [{title: "Arguments non valable", content: "Seuls les arguments \"-target\" et \"-master\" sont valables"}],
                    cmdCtx.msg.channel, {self: {removeItSelf: true, timeout: 5000}});
        }
